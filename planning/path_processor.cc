...
std::vector<StateType> PathProcessor<StateType>::ProcessPath(
  ...
  drake::log()->debug("Building postprocessing components & helpers...");
  ...
  if (parameters.resample_before_smoothing ||
    ...
    drake::log()->debug("Calling pre-smoothing ResamplePath()...");
    ...
    drake::log()->debug("Resampled path from {} to {} states",
                        starting_length, processed_path.size());
  }

  if (parameters.use_shortcut_smoothing) {
    drake::log()->debug("Calling ShortcutSmoothPath()...");
  ...
  if (parameters.use_shortcut_smoothing &&
      parameters.resample_after_smoothing) {
    drake::log()->debug("Calling post-smoothing ResamplePath()...");
    const size_t starting_length = processed_path.size();
    processed_path = ResamplePath(
  ...
  drake::log()->debug("Postprocessing complete");
