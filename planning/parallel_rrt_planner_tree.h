...
// TODO(calderpg) Remove once CRU KNN changes land.
#include <cmath>
#include <cstdint>
#include <functional>
#include <memory>
...
/// tree storage may be reallocated during the propagation step.
...
class ViewOfParallelRRTPlannerTree {
 public:
  ...
  const StateType& operator[](size_t index) const {
    // During the execution of this method, the nodes vector won't change so
    // we can use the unsafe accessor to avoid copying.
...
int64_t GetParallelRRTNearestNeighbor(
    ...
    const StateDistanceFunction<StateType>& distance_fn, bool use_parallel) {
  ...
  const auto neighbors = common_robotics_utilities::simple_knearest_neighbors::
      GetKNearestNeighbors(view_of_tree, sampled, distance_fn, 1, use_parallel);
