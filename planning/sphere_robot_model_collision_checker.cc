...
SphereRobotModelCollisionChecker::ComputeSphereLocationsInWorldFrame(
  ...
  std::vector<std::unordered_map<GeometryId, SphereSpecification>>
      spheres_in_world_frame(robot_sphere_model_.size());
  for (size_t body_index = 0; body_index < robot_sphere_model_.size();
       body_index++) {
    ...
    for (const auto& [sphere_id, sphere] : spheres) {
      ...
      spheres_in_world_frame.at(body_index).emplace(
          sphere_id, SphereSpecification(p_WSo, sphere.Radius()));
    }
  }
...
void SphereRobotModelCollisionChecker::DoRemoveAddedGeometries(
