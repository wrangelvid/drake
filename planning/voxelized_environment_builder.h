...
inline std::ostream& operator<<(
  ...
  strm << "SDP query " << distance.distance
       << " distance "
       //<< distance.grad_W.transpose() << " gradient " << distance.id_G
       << " geometry";
  return strm;
}
...
voxelized_geometry_tools::CollisionMap BuildCollisionMap(
    ...
    const std::unordered_set<drake::geometry::GeometryId>& geometries_to_ignore,
