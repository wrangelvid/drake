...
MbpEnvironmentCollisionChecker::AddEnvironmentCollisionShapeToBody(
    const std::string&, const drake::multibody::Body<double>& bodyA,
  ...
  //drake::log()->debug(
  //    fmt::format("Adding shape (group: [{}]) to {} (FrameID {}) at X_AG =\n{}",
  //    //group_name, bodyA.scoped_name().to_string(), body_frame_id.value(),
  //    group_name, "test", body_frame_id.value(),
  //    X_AG.GetAsMatrix4()));
