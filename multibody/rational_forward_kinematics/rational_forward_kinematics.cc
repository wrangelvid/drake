...
void RationalForwardKinematics::
  ...
  CalcChildPose(X_AP.R_AB, X_AP.p_AB, X_PF.GetAsIsometry3(), X_MC.GetAsIsometry3(), R_FM, p_FM, &(X_AC->R_AB),
                &(X_AC->p_AB));
...
void RationalForwardKinematics::
  ...
  if (dynamic_cast<const internal::RevoluteMobilizer<double>*>(mobilizer) !=
    ...
    math::RigidTransform<double> X_PF;
    math::RigidTransform<double> X_MC;
    ...
    math::RigidTransform<double> X_FM;
    math::RigidTransform<double>  X_PF;
    math::RigidTransform<double>  X_MC;
    if (!is_order_reversed) {
    ...
    CalcLinkPoseWithWeldJoint(X_FM.GetAsIsometry3(), X_PF.GetAsIsometry3(), X_MC.GetAsIsometry3(), X_AP, X_AC);
  } else if (dynamic_cast<const internal::SpaceXYZMobilizer<double>*>(
                 mobilizer) != nullptr) {
    throw std::runtime_error("Gimbal joint has not been handled yet.");
